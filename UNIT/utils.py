...
from UNIT.networks import Vgg16, ResNet
...
def load_vgg16(model_dir):
    """Load the VGG16 model, using PyTorch's torchvision models."""
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    weight_path = os.path.join(model_dir, 'vgg16.pth')

    if not os.path.exists(weight_path):
        # Download the pre-trained VGG16 model from torchvision
        vgg = models.vgg16(pretrained=True)
        torch.save(vgg.state_dict(), weight_path)
    else:
        vgg = models.vgg16()
